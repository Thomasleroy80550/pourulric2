        ...
        {!loadingTasks && !error && userRooms.length === 0 ? (
          ...
          <div className="grid-container" style={{
            ...
            {userRooms.map((room, roomIndex) => {
              console.log("DEBUG BookingPlanningGrid: Processing user room:", room.room_name, "with ID:", room.room_id);
              return (
                <React.Fragment key={room.id}>
                  ...
                  {reservations
                    .filter(reservation => {
                      const matches = reservation.property_name === room.room_name || reservation.krossbooking_room_id === room.room_id;
                      console.log(`DEBUG BookingPlanningGrid: Checking reservation ${reservation.id} (${reservation.guest_name}) - property_name: ${reservation.property_name}, krossbooking_room_id: ${reservation.krossbooking_room_id} against user room ${room.room_name} (ID: ${room.room_id}). Matches: ${matches}`);
                      return matches;
                    })
                    .map((reservation) => {
