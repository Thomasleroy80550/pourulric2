...
const StatusBar = ({
  ...
  <div className="flex flex-nowrap items-center gap-4 overflow-x-auto">
    {loading ? (
      <span className="text-[11px] text-gray-400">Vérification des services…</span>
    ) : (
      statuses.map((s) => {
        const dotClass =
  ...
  return (
    <div className="notranslate min-h-[100svh] flex bg-white" translate="no">
      ...
      >
        ...
        <div className="absolute -bottom-40 -left-24 h-[28rem] w-[28rem] rounded-full bg-white/5 pointer-events-none" />
        <div
        ...
        <div className="relative z-10 max-w-md">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/15 px-3 py-1 text-xs font-medium text-white/90 backdrop-blur">
            <Sparkles className="h-3.5 w-3.5" />
            Plateforme de gestion locative
          </span>
          <h2 className="mt-6 text-4xl xl:text-[2.75rem] font-bold leading-[1.15] tracking-tight">
            Pilotez vos locations comme une agence pro.
          </h2>
          <p className="mt-4 text-white/70 text-base leading-relaxed">
            Réservations, revenus, documents et conciergerie — tout votre
          ...
          <ul className="mt-10 space-y-5">
            {features.map((f) => (
              <li key={f.title} className="flex items-start gap-4">
                <span className="mt-0.5 inline-flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-white/10 border border-white/15">
        ...
        <div className="relative z-10">
          <div className="rounded-2xl bg-white/10 border border-white/15 p-5 backdrop-blur">
            <p className="text-sm leading-relaxed text-white/85">
              “Une visibilité totale sur mes biens et mes revenus. Je gagne des
      ...
      {/* Colonne formulaire */}
      <main className="flex-1 flex flex-col min-h-[100svh] pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
