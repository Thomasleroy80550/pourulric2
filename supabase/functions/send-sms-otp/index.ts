...
serve(async (req) => {
  ...
  try {
    const { phoneNumber, mode } = await req.json();
    if (!phoneNumber) {
    ...
    // Auth requise uniquement pour le mode 'profile'
    if (flowMode === 'profile') {
    ...
    const bodyParams = new URLSearchParams({ To: normalizedPhone, Channel: 'sms' });
